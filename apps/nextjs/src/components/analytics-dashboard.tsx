...
export function AnalyticsDashboard() {
  ...
  const getDemoAnalytics = useCallback(
    (): AnalyticsData => ({
      dailyRevenue: [
        {
      ...
      recentPayments: [
        {
          transaction_hash:
            "0x826f1ce5d3606307e766dfaa72ad8e7bbbfe909f4b4b44ffb2d02e4ac39574e7",
  ...
  const totalRevenue = analytics.dailyRevenue.reduce(
    (sum, day) => sum + day.daily_revenue_eth,
    0
  );
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg">
        ...
        <div className="space-y-3">
          {analytics.recentPayments.slice(0, 5).map((payment) => (
            <div
              key={payment.transaction_hash}
              className="flex justify-between items-center py-2 border-b border-gray-100"
            >
              <div>
                <p className="text-sm font-mono text-gray-600">
                  {payment.from_address.slice(0, 6)}...
                  {payment.from_address.slice(-4)}
                </p>
